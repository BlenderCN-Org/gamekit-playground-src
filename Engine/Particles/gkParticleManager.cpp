...
public:
	...
	void initialize(void)
	{
		try
		{
			...
			Ogre::MaterialPtr mat = Ogre::MaterialManager::getSingleton().create(DEFAULT_HALO_MAT, GK_BUILTIN_GROUP).staticCast<Ogre::Material>();
	...
	gkString createOrRetrieveHaloMaterial(const gkString& baseMatName)
	{
		...
		try
		{
			...
			{
				Ogre::MaterialPtr baseMat = mmgr.getByName(baseMatName).staticCast<Ogre::Material>();
				if (!baseMat.isNull())
				{
					Ogre::MaterialPtr mat = mmgr.create(haloMatName, baseMat->getGroup()).staticCast<Ogre::Material>();
