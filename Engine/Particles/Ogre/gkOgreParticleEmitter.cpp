...
void gkOgreParticleEmitter::_initParticle(Particle* pParticle)
{
	...
	pParticle->mDirection = props.m_velocity;
		...
		if (!any.isEmpty())
		{
			...
			if (mesh)
			{
				...
				if (props.m_emitfrom == gkParticleSettingsProperties::EF_VERTS)
				{
					...
					pParticle->mPosition = v.co;
					
					if (props.m_velNormal != 0)
						pParticle->mDirection = (pParticle->mDirection + v.no * props.m_velNormal)/2;
				}
				else // if (props.m_emitfrom == gkParticleSettingsProperties::EF_FACES)
				{
					...
					pParticle->mPosition = randomInTri(face.p);

					if (props.m_velNormal != 0)
						pParticle->mDirection = (pParticle->mDirection + face.normal() * props.m_velNormal)/2;
				}
			}
		}
	}
	...
	if (data)
		data->m_initDir = pParticle->mDirection;
	...
	pParticle->mTimeToLive = pParticle->mTotalTimeToLive = props.m_lifetime; //genEmissionTTL();
}
