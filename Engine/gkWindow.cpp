...
#if GK_PLATFORM != GK_PLATFORM_EMSCRIPTEN
...
//		params["externalGLContext"] = Ogre::StringConverter::toString(0);
		...
		m_mouse.winsize.x = m_rwindow->getWidth();
		m_mouse.winsize.y = m_rwindow->getHeight();
...
bool gkWindow::mouseMoved(const OIS::MouseEvent& arg)
{
	...
	// inject to optional ois-listener if present
	if (m_OISMOUSE_OPT)
		 m_OISMOUSE_OPT->mouseMoved(arg);
	return true;
}
...
bool gkWindow::mousePressed(const OIS::MouseEvent& arg, OIS::MouseButtonID id)
{
	...
	// inject to optional ois-listener if present
	if (m_OISMOUSE_OPT)
		 m_OISMOUSE_OPT->mousePressed(arg,id);
...
bool gkWindow::mouseReleased(const OIS::MouseEvent& arg, OIS::MouseButtonID id)
{
	...
	// inject to optional ois-listener if present
	if (m_OISMOUSE_OPT)
		 m_OISMOUSE_OPT->mouseReleased(arg,id);
...
bool gkWindow::keyPressed(const OIS::KeyEvent& arg)
{
	...
	// call optional OIS-Listener
	if (m_OISKEYBOARD_OPT) m_OISKEYBOARD_OPT->keyPressed(arg);
...
bool gkWindow::keyReleased(const OIS::KeyEvent& arg)
{
	...
	// call optional OIS-Listener
	if (m_OISKEYBOARD_OPT) m_OISKEYBOARD_OPT->keyReleased(arg);
...
bool gkWindow::buttonPressed(const OIS::JoyStickEvent& arg, int button)
{
	...
	if (m_OISJOYSTICK_OPT) m_OISJOYSTICK_OPT->buttonPressed(arg,button);
...
bool gkWindow::buttonReleased(const OIS::JoyStickEvent& arg, int button)
{
	...
	if (m_OISJOYSTICK_OPT) m_OISJOYSTICK_OPT->buttonReleased(arg,button);
...
bool gkWindow::axisMoved(const OIS::JoyStickEvent& arg, int axis)
{
	...
	if (m_OISJOYSTICK_OPT) m_OISJOYSTICK_OPT->axisMoved(arg,axis);
...
void gkWindow::windowResized(Ogre::RenderWindow* rw)
{
	...
	gkMessageManager::getSingleton().sendMessage(GK_MSG_GROUP_INTERNAL,"",GK_MSG_INTERNAL_GAME_WINDOWRESIZED,"");
...
#ifdef OGREKIT_COMPILE_LIBROCKET
	if (m_gui){
		gkLogger::write("Librocket-dim:"+gkToString(getWidth())+"*"+gkToString(getHeight()));

		Rocket::Core::Vector2i dim(m_gui->getContext()->GetDimensions());
		m_gui->getContext()->SetDimensions(Rocket::Core::Vector2i(getWidth(), getHeight()));
		m_gui->onWindowResize();
		m_gui->getContext()->Update();
		for (int i=0; i < m_gui->getContext()->GetNumDocuments(); i++)
		{
