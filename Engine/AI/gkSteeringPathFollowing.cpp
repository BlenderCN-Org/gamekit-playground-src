...
gkSteeringPathFollowing::gkSteeringPathFollowing(gkGameObject* obj, gkScalar maxSpeed, const gkVector3& forward, const gkVector3& up, const gkVector3& side)
...
bool gkSteeringPathFollowing::createPath()
{
	...
	return m_navPath->createFromRecast(m_navMesh, m_obj->getPosition(), m_goalPosition, m_polyPickExt, m_maxPathPolys, radius());
#else
	gkLogger::write("No path available!");
...
bool gkSteeringPathFollowing::steering(STATE& newState, const float elapsedTime)
{
	...
	gkScalar predictionTime = maxSpeed() / targetSpeed * 1.25;
