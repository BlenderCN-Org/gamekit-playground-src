...
//		gkLogger::write("NavData new:"+m_object->getName()+" idx:"+gkToString(triangleIndex)+" vertidx:"+gkToString((int)n));
...
//			return;
		...
		size_t tBaseIndex = m_object->getNavData().triangleBaseIndex ;
		size_t vBaseIndex = tBaseIndex / 2 + triangleIndex * 3;
		size_t nBaseIndex = tBaseIndex / 6;

		gkLogger::write("NavData reuse:"+m_object->getName()+" idx:"+gkToString(triangleIndex)+" vertidx:"+gkToString((int)vBaseIndex));
...
void gkNavMeshData::AddCollisionObj()
{
	...
	case SPHERE_SHAPE_PROXYTYPE:
		{
			// Sphere shape is not taken into account,yet
			return;
		}
	...
	if (m_object->hasVariable("gk_no_navmesh") && m_object->getNavData().isEmpty())
	{
