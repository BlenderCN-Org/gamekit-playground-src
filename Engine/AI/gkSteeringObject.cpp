...
void gkSteeringObject::calcSteeringForce(const OpenSteer::Vec3& force, const float elapsedTime,gkVector3& direction, gkQuaternion& rot)
{
	...
	Vec3 newVelocity = velocity() + newAcceleration * elapsedTime * 5;
	...
	direction = m_forward * m_speed;
	rot = m_steerUtility.getRotation(forward(), newVelocity.normalize());
...
void gkSteeringObject::applySteeringForce(const Vec3& force, const float elapsedTime)
{
	...
	calcSteeringForce(force,elapsedTime,dir,rot);

	m_obj->rotate(rot, TRANSFORM_LOCAL);

	gkScalar oldZ = m_obj->getLinearVelocity().z;
	//dir.z = dir.z + oldZ;
	dir.z=oldZ;
	m_obj->setLinearVelocity(dir,TRANSFORM_LOCAL);
...
Vec3 gkSteeringObject::steerToEvadeOthers(gkGameObject* target)
{
