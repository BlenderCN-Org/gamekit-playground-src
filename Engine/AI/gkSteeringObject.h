...
public:
	...
	virtual bool steering(STATE& newState, const float elapsedTime) { return false;};
	virtual void reset();
	...
	void calcSteeringForce(const OpenSteer::Vec3& force, const float elapsedTime,gkVector3& direction, gkQuaternion& rot);
	...
	OpenSteer::Vec3 steerToEvadeOthers(gkGameObject* target = 0);
...
protected:
    ...
    virtual void annotatePathFollowing (const OpenSteer::Vec3& future,
                                        ...
                                        const float )
    {
    	gkDebugger* visDebug = gkEngine::getSingleton().getActiveScene()->getDebugger();
    	if (visDebug)
    	{
//    		visDebug->drawLine(position(),position()+gkVector3(3,0,0),gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+gkVector3(0,6,0),gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+gkVector3(0,0,9),gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+target,gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+target,gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+target,gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+onPath,gkVector3(1,1,1));
//    		visDebug->drawLine(position(),position()+future,gkVector3(1,1,1));
    	}
    }
    ...
    virtual void annotateAvoidObstacle (const float minDistanceToCollision)
    {
    	...
    	if (visDebug)
    	{
    		visDebug->drawCircle(position(),minDistanceToCollision,gkVector3(0.8f,0.08f,0.0f));
    	}
    }
