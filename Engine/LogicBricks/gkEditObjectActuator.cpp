...
void gkEditObjectActuator::suspendDynamics(void)
{
	if (m_object)
	{
		// only suspend if the object isn't already suspended. (that would crash, cause suspending=no attached physics object)
		if (m_object->getPhysicsController() && !m_object->getPhysicsController()->isSuspended())
