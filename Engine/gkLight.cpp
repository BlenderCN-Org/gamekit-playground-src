...
void gkLight::setPowerScale(float powerScale) {
	if (m_light)
		m_light->setPowerScale(powerScale);
}

float gkLight::getPowerScale() {
	if (m_light)
...
void gkLight::sendNotification(const Notifier::Event& e) {
	gkGameObject::sendNotification(e);

	if (e == gkGameObject::Notifier::UPDATED) {
