...
gkVariable::gkVariable(const gkColor& v, const gkString& name)
	:    m_value((int)0),
	     m_type(VAR_NULL),
	     m_name(""),
	     m_debug(false), m_lock(false)
...
void gkVariable::setValue(const gkColor& v)
{
	if (!m_lock)
	{
		m_type = VAR_COLOR;
...
gkColor gkVariable::getValueColor(void) const
{
	return m_value.get<gkColor>(gkColor(0,0,0));
}
...
bool gkVariable::operator == (const gkVariable& o) const
{
	...
	case VAR_COLOR: return getValueColor()     == o.getValueColor();
	default:
		return getValueString() == o.getValueString();
	}
	return false;
