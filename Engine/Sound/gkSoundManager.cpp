...
bool gkSoundManager::loadSound(const gkString& soundFile,const gkString& soundName, const gkString& groupName)
{
	gkPath pth(soundFile);

	if (pth.exists())
	{
		gkSound* sndObj = create<gkSound>(gkResourceName(soundName.empty()?pth.base():soundName,groupName));
		utMemoryStream* stream = pth.getAsStream();
		int size = stream->size();
		bool didTheJob = sndObj->load(stream->ptr(),stream->size());
		delete stream;
		if (didTheJob)
			return true;
		else
...
void gkSoundManager::stop() {
	if (m_stream) {
		m_stream->pause();
	}
}

void gkSoundManager::restart() {
	if (m_stream) {
		m_stream->resume();
	}
}
