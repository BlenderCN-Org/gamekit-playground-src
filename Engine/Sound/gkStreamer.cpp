...
void gkStreamer::playSound(gkSource* snd)
{
	...
	if (snd)
	{
//		gkLogger::write("QUEUE SOUND!",true);
		_LOG_FOOT_
		m_wantsQSync = true;
		...
		m_qsync.wait();
		if (!snd->isBound()) {
			// Start new buffer and add to queue.
			m_queueBuffers.push_back(new gkBuffer(snd));
		}
		else {
			// play a buffered source
			if (snd){
				alSourcePlay(snd->getBuffer()->m_source);
			}
			m_queueBuffers.push_back(snd->getBuffer());
		}
	}
}
...
void gkStreamer::pause()
{
	m_wantPause = true;

}

void gkStreamer::resume()
{
	m_wantPause = false;
	gkLogger::write("resume",true);
	m_pausesync.signal();
	gkLogger::write("after resume",true);
}
	...
	while (isRunning())
	{
		...
		try
		{
			if (m_wantPause){
				...
				float beforePauseVolume = gkSoundManager::getSingleton().getGlobalVolume();
				gkSoundManager::getSingleton().setGlobalVolume(0);
				// wait till I get signaled to go on again
				m_pausesync.wait();
				// reset volume
				gkSoundManager::getSingleton().setGlobalVolume(beforePauseVolume);


				gkLogger::write("end pause!",true);
			}
