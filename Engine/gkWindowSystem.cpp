...
#ifdef OGREKIT_BUILD_BLACKBERRY
#include "gkWindowBlackBerry.h"
#endif

#if GK_PLATFORM == GK_PLATFORM_EMSCRIPTEN
# include "Emscripten/gkWindowEmscripten.h"
#endif
...
gkWindow* gkWindowSystem::createWindow(const gkUserDefs& prefs)
{
	...
	gkWindow* window = new gkWindowBlackBerry();
#elif GK_PLATFORM == GK_PLATFORM_EMSCRIPTEN
	gkWindow* window = new gkWindowEmscripten();
#else
	gkWindow* window = new gkWindow();
#endif

	gkPrintf("try to create window!");
