...
void gkFontManager::parseScript(utMemoryStream* buffer)
{
	...
	while (niter.hasMoreElements())
	{
		...
		Ogre::FontPtr fp = Ogre::FontManager::getSingleton().getByName(name).staticCast<Ogre::Font>();
		if (!fp.isNull())
		{
			// use it
			return;
		...
		try
		{
			...
			fp = Ogre::FontManager::getSingleton().create(name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, true, fl).staticCast<Ogre::Font>();
