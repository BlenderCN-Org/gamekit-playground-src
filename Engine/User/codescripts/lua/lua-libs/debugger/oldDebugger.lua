...
-- if the env var is nil, lua debuggingthe debugger will try to guess it.
--
...
--        Gk.log("make_level:"..tostring(info.source).." line:"..info.currentline.." level:"..level)
        --local uri = platform.get_uri(info.source)
        local uri = DEBUG_URI..info.source
--        Gk.log("URI:"..uri)
...
--    Gk.log("Open Path:"..tostring(path))
...
--        Gk.log("NormalizedFilenname:"..bp.filename)
...
local function line_hook(line)
    ...
    if (not string.find(info.source,"debugger")) then
	    uri = DEBUG_URI..info.source
    else
--    	Gk.log("Prevented debugger:"..info.source)
    end
--    Gk.log("line-hook uri:"..tostring(uri).." line:"..line.." infosrc:"..info.source)
