...
Gk.import(LUA_LIB_DIR.."/thrift/TProtocol.lua")
...
function TBinaryProtocol:writeMessageBegin(name, ttype, seqid)
  if self.strictWrite then
    ...
    if not self.serviceName then
      ...
      Gk.log("->ServerCall:"..name)
    else
      self:writeString(self.serviceName..":"..name)
      Gk.log("->ServerCall:"..self.serviceName..":"..name)
    end
