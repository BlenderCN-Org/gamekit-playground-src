	...
	:   m_bscene(sc), m_gscene(0), m_file(fp), m_groupName(fp->getResourceGroup())
{
	...
	{
		new gkNodeTreeConverter();
	}
	gkNodeTreeConverter::getSingletonPtr()->process(fp);
}
...
float gkBlenderSceneConverter::retrieveCustomPropertyFloat(const gkString& key,Blender::IDProperty* idprop,float defaultValue)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key);
		return defaultValue;
	}

	float f = (*(float *)&(prop)->data.val);
	return f;
}

void gkBlenderSceneConverter::retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,gkVector3& defaultValue)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key,true);
		return;
	}
	...
	{
		if (prop->len!=3) {
			gkLogger::write("CustomProp:"+key+" has wrong amount of floats:"+gkToString(prop->len),true);
			return;
		}

		float* array = (float*)prop->data.pointer;
		defaultValue.x = array[0];
		defaultValue.y = array[1];
		defaultValue.z = array[2];
	}
	else
	{
		gkLogger::write("CustomProp:"+key+" is not an floatVec",true);
	}
}

void gkBlenderSceneConverter::retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,gkVector4& defaultValue)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key,true);
...
void gkBlenderSceneConverter::retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,gkVector2& defaultValue)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key,true);
...
void gkBlenderSceneConverter::retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,utArray<float>& floatVec, utArray<float>& defaultArray)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key,true);
		floatVec = defaultArray;
		return;
	}
	floatVec.clear();
	if (prop->subtype == IDP_FLOAT)
...
void gkBlenderSceneConverter::retrieveCustomPropertyIntVec(const gkString& key,Blender::IDProperty* idprop,utArray<int>& intVec,utArray<int>& defaultArray)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key,true);
	...
	{
		intVec.clear();
		int* array = (int*)prop->data.pointer;
		for (int i=0;i<prop->len;i++)
		{
			int val = array[i];
...
int gkBlenderSceneConverter::retrieveCustomPropertyInt(const gkString& key,Blender::IDProperty* idprop,int defaultValue)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key);
	...
	int result = prop->data.val;
	return result;
}


gkString gkBlenderSceneConverter::retrieveCustomPropertyString(const gkString& key,Blender::IDProperty* idprop,const gkString& defaultValue)
{
	Blender::IDProperty* prop = findProperty(key,idprop);
	if (!prop) {
		gkLogger::write("Unknown customproperty:"+key);
	...
	gkString result = gkString((char*)prop->data.pointer);
	return result;
}

gkVariable gkBlenderSceneConverter::getCustomVariable(const gkString& name,Blender::IDProperty* startPoint)
{
	startPoint = findProperty(name,startPoint);
	if (startPoint)
	{
		gkVariable result;
		if (startPoint->type == IDP_FLOAT)
		{
			float value = retrieveCustomPropertyFloat(name,startPoint);
			result.setValue(value);
			return result;
		}
		else if (startPoint->type == IDP_INT)
		{
			int value = retrieveCustomPropertyInt(name,startPoint);
			result.setValue(value);
			return result;
		}
		else if (startPoint->type == IDP_STRING)
		{
			gkString value = retrieveCustomPropertyString(name,startPoint);
			result.setValue(value);
			return result;
		}
		else if (startPoint->type == IDP_ARRAY)
		{
			utArray<float> arr;
			utArray<float> defaultArr;
			retrieveCustomPropertyFloatVec(name,startPoint,arr,defaultArr);
			if (arr.size()==0)
			{
				result.setValue(0,"");
				return result;
			...
			{
				gkVector2 vec2(arr[0],arr[1]);
				result.setValue(vec2);
				return result;
			}
			else if (arr.size()==3)
			{
				gkVector3 vec3(arr[0],arr[1],arr[2]);
				result.setValue(vec3);
				return result;
			}
			else if (arr.size()==4)
			{
				gkVector4 vec4(arr[0],arr[1],arr[2],arr[3]);
				result.setValue(vec4);
				return result;
			}
			else
			{
				gkLogger::write("got array with "+gkToString((int)arr.size())+" items! not sure how to handle this");
				result.setValue(0,"");
				return result;
			}
		}
		...
		{
			gkLogger::write("Unsupported Type in getCustomVariable:"+gkToString(startPoint->type));
		}
...
Blender::IDProperty* gkBlenderSceneConverter::findProperty(const gkString& name,Blender::IDProperty* startPoint)
{
	if (!startPoint)
		return 0;

	gkString currentName = startPoint->name;

	if (currentName == name) {
		return startPoint;
	}
	else if (currentName == "_RNA_UI") {
		// Ignore _RNA_UI-Data since they are not complete
		return 0;
	}

	if (startPoint->type == IDP_GROUP)
	{
		Blender::ListBase* list = &startPoint->data.group;
		Blender::IDProperty* prop = (Blender::IDProperty*)list->first;


		while (prop)
		{
			Blender::IDProperty* propResult = findProperty(name,prop);
			if (propResult) {
				return propResult;
			}
			prop = prop->next;
...
void gkBlenderSceneConverter::retrieveCustomProperties(Blender::IDProperty* idprop,CPValue& result)
{
	if (!idprop)
		return;
	...
	if (idprop->type == IDP_GROUP)
	{
		...
		Blender::ListBase* list = &idprop->data.group;
		Blender::IDProperty* prop = (Blender::IDProperty*)list->first;

		while (prop)
		{
			CPValue newMapElem;
			retrieveCustomProperties(prop,newMapElem);
			result.map.insert(newMapElem.name.c_str(),newMapElem);
			prop = prop->next;
		}
	...
	{
		result.name = idprop->name;
		result.stringValue = gkString((char*)idprop->data.pointer);
		result.type = cpt_string;
	}
	else if (idprop->type == IDP_FLOAT)
	{
		result.name = idprop->name;
		result.floatValue = idprop->data.val;
		result.type = cpt_float;
	}
	else if (idprop->type == IDP_INT)
	{
		result.name = idprop->name;
		result.intValue = idprop->data.val;
		result.type = cpt_int;
	}
	else if (idprop->type == IDP_IDPARRAY)
	{
		int len = idprop->len;
		Blender::IDProperty* array = (Blender::IDProperty*)idprop->data.pointer;
		gkString name(idprop->name);
		result.name = name;
		result.type = cpt_list;

		for (int i=0;i<len;i++)
		{
			CPValue listElem;
			retrieveCustomProperties(&array[i],listElem);
			result.list.push_back(listElem);
		}
	}
	else if (idprop->type == IDP_ARRAY)
	{
		int len = idprop->len;
		...
		{
			float* array = (float*)idprop->data.pointer;
		...
		{
			int* array = (int*)idprop->data.pointer;
		...
		{
			gkLogger::write("CPArrayType:"+gkToString(idprop->subtype)+" not implemented!");
		}

//		for (int i=0;i<len;i++)
//		{
//			CPMap innerResult;
//			retrieveCustomProperties(&array[i],innerResult);
//			gkString* inner = innerResult.get("");
//			if (inner)
//				resultMap.insert(gkString(name+"_"+gkToString(i)).c_str(),*inner);
//		}
//		resultMap.insert(name.c_str(),gkToString(len));
	}
}

bool gkBlenderSceneConverter::customPropertyListContains(const CPValue& list,const gkString& field,const gkString& value)
{
	// THIS IS A MESS. Do it somehow else. Didn't want to use pointers cause cleaning up would be a horror (time to go for sharedptrs)
	if (list.type == cpt_list)
	{
		for (int i=0;i<list.list.size();i++)
		{
			const CPValue& map = list.list.at(i);
			int pos;
			if (map.type == cpt_map)
			{
				for (int j=0;j<map.map.size();j++)
				{
					gkString key = map.map.keyAt(j).key();
					if (key == field)
					{
						const CPValue& elem = map.map.at(j);
						if (elem.stringValue == value)
						{
							return true;
						}
					}
			...
			{
				gkLogger::write("Unsupported access in customPropertyListContains",true);
			}
		}
		return false;
	}

//	gkLogger::write("Did try to use customPropertyListContains, but it was not supported this way!",true);
	return false;
}


const gkBlenderSceneConverter::CPValue*  gkBlenderSceneConverter::getCustomPropertyMapElement(const CPValue* map,const gkString& mapkey)
{
	if (map->type == cpt_map)
	{
		for (int j=0;j<map->map.size();j++)
...
const gkBlenderSceneConverter::CPValue* gkBlenderSceneConverter::getCustomPropertyMapFromList(const CPValue& list,const gkString& field,const gkString& value)
{
	// THIS IS A MESS. Do it somehow else. Didn't want to use pointers cause cleaning up would be a horror (time to go for sharedptrs)
	if (list.type == cpt_list)
	{
...
void gkBlenderSceneConverter::convertWorld(void)
{
	...
	if (m_bscene->world)
	{
		...
		sprops.m_pbsRequested = retrieveCustomPropertyInt("gk_activate_pbs",m_bscene->id.properties,true);
...
void gkBlenderSceneConverter::convertGroups()
{
	...
	while (iter.hasMoreElements())
	{
		...
		gkVector3 dupliOffest(bgrp->dupli_ofs[0],bgrp->dupli_ofs[1],bgrp->dupli_ofs[2]);
		...
		for (Blender::GroupObject* bgobj = (Blender::GroupObject*)bgrp->gobject.first; bgobj; bgobj = bgobj->next)
		{
			if (bgobj->ob)
			{
				...
				if ( (bobj->transflag& OB_DUPLIGROUP) && bobj->dup_group != 0)
				{
					...
					gobj->getProperties().m_transform.loc = gobj->getProperties().m_transform.loc - dupliOffest;
					// Owning group
...
void gkBlenderSceneConverter::convertGroupInstances()
{
	...
	if (m_gscene)
	{
		...
		while (it.hasMoreElements())
		{
			...
			const gkResourceName groupName(GKB_IDNAME(bgobj),
										bgobj->id.lib? gkBlendLoader::getSingleton().getLibraryGroup(gkBlendFile::getLibraryPath(bgobj->id.lib->filepath))
													: m_groupName);
			...
			{
				gkLogger::write("unknown group:" + groupName.getFullName(),true);
			}
		}
	} else
	{
...
void gkBlenderSceneConverter::convertObject(Blender::Object* bobj, gkGameObject* gobj)
{
	...
	if (gobj == 0)
	{
		...
		if (m_gscene->hasObject(name) || gkGameObjectManager::getSingleton().getByName(gkResourceName(name.str(),m_gscene->getGroupName()))!=0 )
		{
			gkString newName(gkUtils::getUniqueName("_renamed_"+name.str()));
			gkLogger::write("###### ERROR! CAUTION!! #####",true);
			gkLogger::write("Duplicated element:"+name.str()+"! Renaming it to -> "+newName,true);
			gkLogger::write("Did you link a file? Make sure that there are no scene-name and object-name collisions!!",true);
			gkLogger::write("###### your application might not work properly anymore!! ######",true);

			gkDebugScreen::printTo("###### ERROR!! CAUTION!!!! #######");
			gkDebugScreen::printTo("Duplicated element:"+name.str()+"! Renaming it to -> "+newName);
			gkDebugScreen::printTo("Did you link a file? Make sure that there are no scene-name and object-name collisions!!");
			gkDebugScreen::printTo("###### your application might not work properly anymore!! ######");
	...
	if (gobj)
	{
		...
		convertNodeTree(gobj, bobj);
	}
}


void gkBlenderSceneConverter::convertNodeTree(gkGameObject* gobj, Blender::Object* bobj)
{
	gkString nodeTreeValue = retrieveCustomPropertyString("gk_nodetree",bobj->id.properties);
	if (nodeTreeValue!="") {
		gobj->setLogicTreeName(nodeTreeValue);
	} else {
		gkLogger::write("NO NODETREE");
	}
}
...
// TODO: Experiment: children physic objects.
...
//	const gkResourceName skelName(GKB_IDNAME(bobj), m_groupName);
	const gkResourceName skelName(GKB_IDNAME(armature), m_groupName);
...
#define CALCULATE_START_LAYER(VEC) \
if (VEC.size()!=0)\
{\
	...
	{\
		int val = VEC[i];\
		if (val)\
			startLayer = startLayer + std::pow(2.0f,power);\
		power++;\
	}\
}\
else\
...
void gkBlenderSceneConverter::convert(bool createGroupInstances)
{
	...
	utArray<int> defaultArray(5,0);

	utArray<int> intVec11,intVec12,intVec21,intVec22;
	gkBlenderSceneConverter::retrieveCustomPropertyIntVec("gk_start_layers_1_1",m_bscene->id.properties,intVec11,defaultArray);
	gkBlenderSceneConverter::retrieveCustomPropertyIntVec("gk_start_layers_1_2",m_bscene->id.properties,intVec12,defaultArray);
	gkBlenderSceneConverter::retrieveCustomPropertyIntVec("gk_start_layers_2_1",m_bscene->id.properties,intVec21,defaultArray);
	gkBlenderSceneConverter::retrieveCustomPropertyIntVec("gk_start_layers_2_2",m_bscene->id.properties,intVec22,defaultArray);



	int startLayer = 0;
	int power = 0;
	// sry, for some reason I had the urge to use a macro instead of a method
	CALCULATE_START_LAYER(intVec11);
	CALCULATE_START_LAYER(intVec12);
	CALCULATE_START_LAYER(intVec21);
	CALCULATE_START_LAYER(intVec22);

	if (startLayer!=0)
		m_gscene->setLayer(startLayer,true);
	else
		m_gscene->setLayer((UTuint32)m_bscene->lay,true);
	...
	if (!armatureLinker.empty())
	{
		...
		for (i = 0; i < armatureLinker.size(); ++i)
		{
			...
			if (gobjEn && gobjSk && !gobjEn->getProperties().hasBoneParent())
			{
				...
				{
					gkSkeletonResource* skel = skmgr.getByName<gkSkeletonResource>(GKB_IDNAME(ar));
	...
	case CU_NURBS:
		{
			...
			while (true)
			{
				if (nurb->type==CU_POLY || nurb->type==CU_NURBS)
				{
					// get subcurve-props and create it if not present,yet
					gkCurveProperties* props = obj->getCurveProperties(subcurveNr,true);
