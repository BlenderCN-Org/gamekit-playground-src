...
public:
	...
	enum CPType {
		cpt_int,cpt_string,cpt_float,cpt_bool,cpt_map,cpt_list,cpt_none
	};

	struct CPValue {
		gkString name;
		gkString stringValue;
		int intValue;
		float floatValue;
		CPType type;
		utHashTable<utCharHashKey,CPValue> map;
		utList<CPValue> list;
	};

	typedef utHashTable<utCharHashKey,CPValue> CPMap;
	typedef utList<CPValue> CPList;

	static void retrieveCustomProperties(Blender::IDProperty* idprop,CPValue& resultMap);
	static float retrieveCustomPropertyFloat(const gkString& key,Blender::IDProperty* idprop,float defaultValue=0.0f);
	static void retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,gkVector3& defaultValue);
	static void retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,gkVector4& defaultValue);
	static void retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,gkVector2& defaultValue);
	static void retrieveCustomPropertyFloatVec(const gkString& key,Blender::IDProperty* idprop,utArray<float>& vec,utArray<float>& defaultArray);
	static int retrieveCustomPropertyInt(const gkString& key,Blender::IDProperty* idprop,int defaultValue=0);
	static void retrieveCustomPropertyIntVec(const gkString& key,Blender::IDProperty* idprop,utArray<int>& vec,utArray<int>& defaultArray);
	static gkString retrieveCustomPropertyString(const gkString& key,Blender::IDProperty* idprop,const gkString& defaultValue="");
	static Blender::IDProperty* findProperty(const gkString& name,Blender::IDProperty* startPoint);
	static bool customPropertyListContains(const CPValue& list,const gkString& field,const gkString& value);
	static gkVariable getCustomVariable(const gkString& name,Blender::IDProperty* startPoint);

	static const CPValue* getCustomPropertyMapElement(const CPValue* map,const gkString& key);
	static const CPValue* getCustomPropertyMapFromList(const CPValue& list,const gkString& field,const gkString& value);
...
private:
	...
	void convertNodeTree(gkGameObject* gobj, Blender::Object* bobj);
