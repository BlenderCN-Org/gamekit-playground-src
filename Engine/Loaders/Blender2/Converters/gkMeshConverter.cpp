...
void gkBlenderMeshConverter::convertMaterial(Blender::Material* bma, gkMaterialProperties& gma, gkMeshHashKey& hk)
{
	...
	gkString val = gkBlenderSceneConverter::retrieveCustomPropertyString("vp",bma->id.properties);
	if (!val.empty())
	{
		gma.m_vertexProgram = val;
	}

	val = gkBlenderSceneConverter::retrieveCustomPropertyString("fp",bma->id.properties);
	if (!val.empty())
	{
		gma.m_fragmentProgram = val;
	}

	val = gkBlenderSceneConverter::retrieveCustomPropertyString("ignore_blender_tex",bma->id.properties);
	if (!val.empty())
	{
		gma.m_ignoreBlenderTextures = val == "true";
	}
	...
	if (bma->mtex != 0)
	{
		...
		for (int i = 0; i < MAX_MTEX; i++)
		{
			...
			if (bma->mtex[i]->tex->type == TEX_IMAGE)
			{
				...
				// check if the texture is activated via checkbox
				gte.m_isActive = ((1 << i) & bma->septex) == 0;
				gte.m_image = imageName;
				gte.m_name = GKB_IDNAME(ima);
				...
				const Blender::ImageUser& user = mtex->tex->iuser;
				gte.m_isCyclic = user.cycl;
				gte.m_currentFrame =  user.framenr;
				gte.m_framesToUse = user.frames;
				gte.m_startFrame = user.sfra;
			}
		}
	}
}
...
bool gkBlenderMeshConverter::convert(void)
{
	...
	// set the dimension
	m_gmesh->_setDimension(gkVector3(m_bmesh->size)*2);
    ...
    int& validLayers)
{
	...
	if (layers)
	{
		...
		for (int i = 0; i < mesh->pdata.totlayer && validLayers < 8; i++)
		{
			...
			else if (layers[i].type == CD_RECAST)
			{
				// load the data!?
			}
		}
