...
void gkAnimationLoader::convert25AnimData(gkGameObject* obj, Blender::AnimData* adt, gkScalar animfps)
{
	...
	{
		Blender::NlaTrack* nlaTrack = (Blender::NlaTrack*) adt->nla_tracks.first;

		gkString seqName("NLA_"+obj->getName());
		gkAnimationSequence* m_nlaAnim = gkAnimationManager::getSingleton().createAnimationSequence(seqName);
//		m_animations[GK_ANIM_COMBO_ATTACK]->setMode(AK_ACT_END); //not LOOP

		gkResourceName name(nlaTrack->name, m_groupName);
		while (nlaTrack)
		{
			Blender::NlaStrip* nlaStrip = (Blender::NlaStrip*) nlaTrack->strips.first;
			while (nlaStrip) {
				Blender::bAction* act = nlaStrip->act;
				gkString animName = GKB_IDNAME(act);
				gkAnimation* animation = gkAnimationManager::getSingleton().getByName<gkAnimation>(animName);
				gkScalar length = animation->getInternal()->getLength();

				for (int i=0;i<nlaStrip->repeat;i++)
				{

					m_nlaAnim->addItem(animName,      nlaStrip->start / animfps + i*length,    nlaStrip->start / animfps + (i+1)*length,nlaStrip->blendin/animfps,nlaStrip->blendout / animfps);
				}

				gkString stripName = nlaStrip->name;
				nlaStrip = nlaStrip->next;
			}
			nlaTrack = nlaTrack->next;
		}
		gkAnimationPlayer* play =obj->addAnimation(m_nlaAnim,seqName);
