...
gkBone::gkBone(const gkString& name)
	:    m_name(name),  m_bind(), m_parent(0), m_ogrebone(0)
{
	m_bind.setIdentity();
	m_pose.setIdentity();
...
void gkBone::applyChannelTransform(const gkTransformState& channel, gkScalar weight)
{
	...
	if (m_ogrebone)
	{
		m_ogrebone->setPosition(m_pose.loc);
		m_ogrebone->setOrientation(m_pose.rot);
		m_ogrebone->setScale(m_pose.scl);
	}
	...
	{
		_applyBoneChildTransform(iter.getNext());
	}

}
void gkBone::_applyBoneChildTransform(gkGameObject* gobj)
{
	const gkMatrix4& boneMat = getTransform();
	gkTransformState boneState(boneMat);

	gkGameObject* attachedObj = gobj;
...
void gkBone::applyPoseTransform(const gkTransformState& pose)
{
	...
	if (m_ogrebone)
	{
		m_ogrebone->setPosition(pose.loc);
		m_ogrebone->setOrientation(pose.rot);
		m_ogrebone->setScale(pose.scl);
	}
}
...
void gkBone::_addOgreBone(Ogre::Bone* bone)
{
	m_ogrebone = bone;
}
