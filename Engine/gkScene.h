...
public:
	...
	gkMesh*           getMesh(const gkHashedString& name, const gkString& = "");
	...
	gkVector2		  getScreenPos(const gkVector3& vec,bool absolute=true,bool outerFrustum=false);

	gkDebugger*       getDebugger(void);
	gkRecastDebugger*	getRecastDebugger(void);

	void    setNavMeshData(PNAVMESHDATA navMeshData);
	void    updateNavMeshData(gkGameObject* gobj=0);
	gkPBSManager* getPBSManager();

#ifdef OGREKIT_COMPILE_RECAST
	typedef gkAsyncResult<PDT_NAV_MESH > ASYNC_DT_RESULT;
	ASYNC_DT_RESULT m_navMeshResult;
	...
	void createNavMesh(const gkRecast::Config& config=gkRecast::Config());
	bool hasNavMesh();

	gkPreparedLuaEvent* m_navmeshEvent;
	void setNavMeshCallback(gkLuaCurState self,gkLuaCurState func);
	void setNavMeshCallback(gkLuaCurState func);

	gkVector3 getNextPointOnNavmesh(const gkVector3& vec,const gkVector3& polyPickExt=gkVector3(2,4,2));
	PDT_NAV_MESH getNavMesh();
#endif
	...
	void      setLayer(UTuint32 v,bool skipObjectCheck=0);
	...
	gkTweenManager* getTweenManager(void) { return m_tweenManager; }
...
private:
	...
	bool					m_logicBrickManager_firstStep;
	gkTweenManager*			m_tweenManager;

	bool			m_initScriptStarted;

	typedef utList<gkVehicle*> gkVehicleList;
	gkVehicleList m_vehicles;
