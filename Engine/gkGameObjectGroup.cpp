...
void gkGameObjectGroup::destroyGameObjectInstances(const gkString& groupName)
{
	m_instanceManager->destroyGroup(groupName);
}
...
void gkGameObjectGroup::createStaticBatches(gkScene* scene)
{
	...
	while (it.hasMoreElements())
	{
		...
		while (instIt.hasMoreElements())
		{
			...
			// TODO: Check why I had this in? Makes no sense to me
			//obj->createInstance();
			...
			if (obj->getType() == GK_ENTITY && !obj->hasVariable("gk_no_batch"))
			{
						...
						|| obj->getPhysicsController()->isStaticObject())
				{
					...
					m_geometry->addEntity(ent->getEntity(true),
