...
void gkTextManager::parseScripts(const gkString& group)
{
	...
	try
	{
		...
		CompositorArray compositorScripts;
		getTextFiles(compositorScripts, TT_COMPOSIT);


#if defined OGREKIT_COMPILE_OGRE_SCRIPTS && OGREKIT_USE_COMPOSITOR
		CompositorArray::Iterator cIter = compositorScripts.iterator();
		while (cIter.hasMoreElements())
		{
			gkTextFile* tf = (gkTextFile*)cIter.getNext();
