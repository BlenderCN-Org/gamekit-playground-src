...
void gkDebugScreen::initialize()
{
	...
	try
	{
		__LINE__;
		__FILE__;
		gkLogger::write("gkdebug",true);
		m_font = new gkBuiltinFont;
		gkLogger::write("gkdebug1",true);
		Ogre::FontPtr fp = Ogre::FontManager::getSingleton().create("<gkBuiltin/Font>", GK_BUILTIN_GROUP, true, m_font).staticCast<Ogre::Font>();
		gkLogger::write("gkdebug2",true);
		fp->load();
		gkLogger::write("gkdebug3",true);
...
gkDebugScreen* gkDebugScreen::getConsole()
{
	if (gConsole != 0)
	{
		return gConsole;
...
void dsPrintf(const char* fmt, ...)
{
	...
	va_start(lst, fmt);
#ifdef __WIN32
	int size = _vsnprintf(buf, 512, fmt, lst);
#else
