...
void gkGameObject::cloneImpl(gkGameObject* clob)
{
	...
	if (!m_logicTreeName.empty())
	{
		clob->m_logicTreeName = m_logicTreeName;
	}
...
void gkGameObject::createInstanceImpl(void)
{
	...
	m_node = parentNode ? parentNode->createChildSceneNode(m_name.getName()+"-"+this->getOwner()->getGroupName())
						: manager->getRootSceneNode()->createChildSceneNode(m_name.getName()+"-"+this->getOwner()->getGroupName());


	MoveableList::Iterator attachedMoveableIter(m_moveableList);
	while (attachedMoveableIter.hasMoreElements())
	{
		Ogre::MovableObject* obj = attachedMoveableIter.getNext();
...
#ifdef OGREKIT_USE_NNODE
	if (!m_logic && !m_logicTreeName.empty())
	{
		gkLogicTree* tree = gkNodeTreeConverter::getSingletonPtr()->createLogicTree( m_logicTreeName,getName());
		if (!tree) {
			gkLogger::write("There is no nodetree with name:"+m_logicTreeName,true);
			return;
		}
		attachLogic(tree);
	}
...
void gkGameObject::notifyUpdate(void)
{
	...
	if (m_rigidBody)
	{
		m_rigidBody->setTransformState(getWorldTransformState());
	}
	else if (m_character)
	{
		m_character->setTransformState(getWorldTransformState());
	}
	else if (m_ghost){
		m_ghost->setTransformState(getWorldTransformState());
	}
	else if (m_vehicle)
	{
		// TODO: check this.
		m_vehicle->setTransform(getWorldTransform());
	}
...
void gkGameObject::setScale(const gkVector3& v)
{
	...
	if (m_node != 0)
	{
		...
		gkPhysicsController* physCtrl = getPhysicsController();
		if (physCtrl)
		{
			btCollisionShape* shp = physCtrl->getShape();
			if (shp)
			{
				shp->setLocalScaling(gkMathUtils::get(v));
			}
		}
		notifyUpdate();
	}
...
void gkGameObject::playAnimation(gkAnimationPlayer* act, gkScalar blend, int mode, int priority)
{
	if (act)
	{
		// override a stop-request on the same animation, since we want to keep playing
		act->overrideStop();
...
void gkGameObject::tweenToPos(const gkVector3& to,float ptime,short ptransition, short pequation, float pdelay )
{
	m_tweenTranslation = getPosition();
	tween::TweenerParam param(ptime*1000,ptransition,pequation,pdelay);
	param.addProperty(&m_tweenTranslation.x,to.x);
	param.addProperty(&m_tweenTranslation.y,to.y);
	param.addProperty(&m_tweenTranslation.z,to.z);
	param.callback = this;
	getOwner()->getTweenManager()->addTween(param);
}

bool gkGameObject::onTweenStep()
{
	setPosition(m_tweenTranslation);
	return true;
};

void gkGameObject::onTweenComplete()
{
	gkLogger::write("Completed Tween:"+getName(),true);
};


void		 gkGameObject::setGravity(const gkVector3& v)
{
	if (isImmovable())
			return;
...
gkVector3    gkGameObject::getGravity(void)
{
	if (isImmovable())
			return gkVector3::ZERO;
...
int gkGameObject::_attachMoveable(Ogre::MovableObject* moveable)
{
	if (moveable && m_moveableList.find(moveable)==0)
	{
		m_moveableList.push_back(moveable);
		m_node->attachObject(moveable);
		return m_moveableList.size()-1;
	}

	return -1;
}

void gkGameObject::_removeMoveables()
{
	m_moveableList.clear();
}

void gkGameObject::_removeMoveable(int pos)
{
	Ogre::MovableObject* obj = m_moveableList.at(pos);
	if (obj)
		m_moveableList.erase(obj);
}

void gkGameObject::bakeTransformState()
{
	gkGameObjectProperties& props = getProperties();
	props.m_transform = getTransformState();
}
