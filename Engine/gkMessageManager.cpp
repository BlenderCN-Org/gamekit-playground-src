...
void gkMessageManager::GenericMessageListener::handleMessage(gkMessageManager::Message* message)
{
	...
	gkString toFilter(m_toFilter);

	// TODO: Check why this isn't filtered out before... For now a hack! The whole method is a mess and needs to be redone!
	if (toFilter.empty() || toFilter == gkString(message->m_to) || (m_acceptEmptyTo && gkString(message->m_to).empty()))
	{
		Message m = *message;
