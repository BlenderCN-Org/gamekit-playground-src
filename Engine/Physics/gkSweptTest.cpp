...
public:
	SweptTestResultCallback(const gkSweptTest::AVOID_LIST& avoidList, const btVector3& fromA, const btVector3& toA, btOverlappingPairCache* pairCache, btDispatcher* dispatcher,bool isPositiveList=false) :
		btCollisionWorld::ClosestConvexResultCallback(fromA, toA),
		m_avoidList(avoidList),
		m_allowedPenetration(0.0f),
		m_pairCache(pairCache),
		m_dispatcher(dispatcher),
		m_isPositiveList(isPositiveList)
	{
	}
	...
	btScalar addSingleResult(btCollisionWorld::LocalConvexResult& convexResult, bool normalInWorldSpace)
	{
		...
		if (!m_isPositiveList && UT_NPOS != m_avoidList.find(const_cast<btCollisionObject*>(convexResult.m_hitCollisionObject)))
			return 1.0f;
...
//		gkLogger::write("swept:"+((gkPhysicsController*)const_cast<btCollisionObject*>(convexResult.m_hitCollisionObject)->getUserPointer())->getObject()->getName());
...
bool gkSweptTest::collides(const Ogre::Ray& ray, gkScalar rayRadius,int filterGroup,int filterMask)
{
	...
	SweptTestResultCallback rayCallback(m_avoidList, rayFrom, rayTo, pWorld->getBroadphase()->getOverlappingPairCache(), pWorld->getDispatcher(),m_isPositiveList);
...
gkGameObject* gkSweptTest::getHitObject() const
{
