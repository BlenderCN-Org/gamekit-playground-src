...
void gkVehicle::setGearBox(gkGearBox* box) {
    if (m_gearBox)
    	delete m_gearBox;
...
void gkVehicle::setDefaultGearBox()
{
...
//		m_raycaster = new btDefaultVehicleRaycaster(m_dynamicWorld->getBulletWorld());
		m_raycaster = new gkDefaultVehicleRaycaster(m_dynamicWorld->getBulletWorld()
													,m_object->getPhysicsController()->getProperties().m_colMask
													,m_object->getPhysicsController()->getProperties().m_colGroupMask);
...
void gkVehicle::setTransform(const gkTransformState& v) { m_object->setTransform(v); }
...
//	gkLogger::write("Steering:"+gkToString(steering)+"|"+gkToString(wheelTorque));
...
//			gkLogger::write("WheelTorque:"+gkToString(wheelTorque),true);
		}
		
		btWheelInfo::RaycastInfo info(btwheel.m_raycastInfo);

		btRigidBody* rb = (btRigidBody*) info.m_groundObject;
		if (rb)
		{
			gkGameObject* groundobj = (gkGameObject*)rb->getUserPointer();

			gkScalar fric = rb->getFriction();

			frictionBrake = fric;
//			gkLogger::write("FricBreak:"+gkToString(i)+"=>"+gkToString(fric));

//			if (groundobj)
//				gkLogger::write("Ground:"+gkToString(i)+" => "+groundobj->getName()+" Fric:"+gkToString(btwheel.m_frictionSlip),true);
//
//			gkLogger::write("Fric:"+gkToString(btwheel.m_frictionSlip),true);
		}

		if (btwheel.m_bIsFrontWheel)
		{
			m_vehicle->setSteeringValue(steering, i);
			m_vehicle->setBrake(frontBrake + frictionBrake, i);
		}
		else
		{
			m_vehicle->setBrake(rearBrake + frictionBrake, i);
		}
		...
		gkQuaternion quat(trans.getRotation().w(), trans.getRotation().x(), trans.getRotation().y(), trans.getRotation().z());
...
//        m_dynamicWorld->getBulletWorld()->debugDrawObject(trans, wheelShape, btVector3(0,1,0));
//        delete wheelShape;
	}
}
...
void* gkDefaultVehicleRaycaster::castRay(const btVector3& from,const btVector3& to, btVehicleRaycasterResult& result)
{
   btCollisionWorld::ClosestRayResultCallback rayCallback(from,to);

   if(m_useFilter)
   {
        rayCallback.m_collisionFilterMask = m_collisionMask;
        rayCallback.m_collisionFilterGroup = m_collisionGroup;
   }
