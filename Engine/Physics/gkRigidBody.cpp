...
//	GK_ASSERT(m_object && m_object->isInstanced() && m_object->isInActiveLayer());
	...
	const gkTransformState& tstate = m_object->getWorldTransformState();
	...
	{
		// since bullet already have internal masks don't use them
		// (eventhough it shouldn't be a problem!?)
//		int reshiftGroupMask = phy.m_colGroupMask << 5;
//		int reshiftColMask = phy.m_colMask << 5;
...
gkVector3   gkRigidBody::getLinearFactor(void)
{
	return gkMathUtils::get(m_body->getLinearFactor());
}

void		gkRigidBody::setLinearFactor(const gkVector3& factor)
{
	m_body->setLinearFactor(gkMathUtils::get(factor));
}

gkVector3   gkRigidBody::getAngularFactor(void)
{
	return gkMathUtils::get(m_body->getAngularFactor());
}

void		gkRigidBody::setAngularFactor(const gkVector3& factor)
{
	m_body->setAngularFactor(gkMathUtils::get(factor));
}

void gkRigidBody::clearForces(void)
{
	m_body->clearForces();
}

void gkRigidBody::setMass(gkScalar mass)
{
	m_body->setMassProps(mass,btVector3(0,0,0));
}

void gkRigidBody::setGravity(const gkVector3& gravity)
{
	m_body->setGravity(gkMathUtils::get(gravity));
}

gkVector3 gkRigidBody::getGravity(void)
{
	return gkMathUtils::get(m_body->getGravity());
}
