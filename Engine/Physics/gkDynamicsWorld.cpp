...
//  //	 no need to delete since this is done in the m_objects-delete-iteration
	m_ghosts.clear();
...
gkVehicle* gkDynamicsWorld::createVehicle(gkGameObject* vehicleObj)
{
	gkVehicle* vehicle = new gkVehicle(m_scene);
	vehicle->setChassisObject(vehicleObj);
	vehicle->setDriveTrain(gkVehicle::DT_ALLWHEEL);
	vehicle->setEngineTorque(330);
	vehicle->setBrakePower(30);
	vehicle->setRearBrakeRatio(0.6);
	vehicle->setMaxSteeringAngle(0.23);
	vehicle->setRuptorRpm(6000);


	gkString tireNames = vehicleObj->getVariable("gkVehicle")->getValueString();

	Ogre::StringVector tires = Ogre::StringUtil::split(tireNames,",");

	Ogre::StringVector::iterator itr;
	for ( itr = tires.begin(); itr != tires.end(); ++itr )
	{
		gkString tireName = *itr;

		gkGameObject* tireObj = 0;
		if (vehicleObj->isGroupInstance())
		{
			tireObj = vehicleObj->getGroupInstance()->getObject(tireName);
		}
		else
		{
			tireObj = m_scene->getObject(tireName);
		}

		if (!tireObj)
		{
			gkLogger::write("WARNING: COULDN'T FIND TIRE:"+tireName+"!! SKIPPING!",true);
			continue;
		}
//		gkString tireName = tireObj->getVariable("gkTire")->getValueString();
		// calculate offset
		gkVector3 offset(tireObj->getWorldPosition() - vehicleObj->getWorldPosition());


//			offset.z = 0.267f;

		float radius = tireObj->hasVariable("radius")
										?tireObj->getVariable("radius")->getValueInt()
										:tireObj->getAabb().getHalfSize().z;


		// TODO: override these values as well
...
//			gkVector3 connectionPoint(0, 0, 0);
		...
		bool isFront = tireObj->hasVariable("isFront")
												?tireObj->getVariable("isFront")->getValueBool()
												:false;

		if (!tireObj->hasVariable("isFront")) {
			gkLogger::write("WARNING: no isFront-property for wheel:"+tireObj->getName());
		}

		vehicle->addWheel(tireObj, radius, offset, wheelDirection, wheelAxle, isFront,
		                    suspensionRestLength, suspensionStiffness, suspensionRelax, suspensionCompression,
		                    friction, rollInfluence, suspensionTravelCm);

		gkLogger::write("Add wheel:"+tireName+" => "+gkToString(offset));
	}

	// check children for wheels
//	gkGameObjectArray::Iterator iter(vehicleObj->getChildren());
//	while (iter.hasMoreElements())
//	{
//		gkGameObject* child = iter.getNext();
//		if (child->hasVariable("gkTire"))
//		{
//			// actually the value itself is not necessary
//			gkString tireName = child->getVariable("gkTire")->getValueString();
//			// calculate offset
//			gkVector3 offset(child->getWorldPosition() - vehicleObj->getWorldPosition());
//
//
////			offset.z = 0.267f;
//
//			float radius = child->hasVariable("radius")
//											?child->getVariable("radius")->getValueInt()
//											:child->getAabb().getHalfSize().z;
//
//
//			// TODO: override these values as well
////			float connectionHeight = 0.267;
...
////			gkVector3 connectionPoint(0, 0, 0);
//
//			bool isFront = child->hasVariable("isFront")
//													?child->getVariable("isFront")->getValueBool()
//													:false;
//
//			if (!child->hasVariable("isFront")) {
//				gkLogger::write("WARNING: no isFront-property for wheel:"+child->getName());
//			}
//
//			vehicle->addWheel(child, radius, offset, wheelDirection, wheelAxle, isFront,
//			                    suspensionRestLength, suspensionStiffness, suspensionRelax, suspensionCompression,
//			                    friction, rollInfluence, suspensionTravelCm);
//
//			gkLogger::write("Add wheel:"+tireName+" => "+gkToString(offset));
...
void gkDynamicsWorld::substep(gkScalar tick)
{
	...
	gkPhysicsControllers::Iterator ghostIter(m_ghosts);
	while (ghostIter.hasMoreElements())
	{
		gkGhost* ghost = static_cast<gkGhost*>(ghostIter.getNext());
		ghost->check();
	}
