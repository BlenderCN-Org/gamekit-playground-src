	...
	case SH_BVH_MESH:
		{
			if (me != 0)
			{
				...
				if (triMesh->getNumTriangles() > 0)
				{
					...
					case SH_GIMPACT_MESH:
					{
//						btGImpactConvexDecompositionShape* gimpactDecompShape = new btGImpactConvexDecompositionShape(triMesh,btVector3(1.f,1.f,1.f,btScalar(0.01)));
//						gimpactDecompShape->setMargin(0.07f);
//						gimpactDecompShape->updateBound();
//						shape = gimpactDecompShape;
...
bool gkPhysicsController::isGhost()
{
	return m_object->getProperties().isGhost();
}
