...
#include "User/gkCodescripts.inl"
#endif
...
#endif
		gkLogger::write("r2",true);

		Ogre::EmbeddedZipArchiveFactory::addEmbbeddedFile("codescripts.zip", CODESCRIPTS, CODESCRIPTS_SIZE, 0);
		mgr.addResourceLocation("codescripts.zip", "EmbeddedZip",  Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,true);

		Ogre::EmbeddedZipArchiveFactory::addEmbbeddedFile("gkmaterials.zip", GKMATERIALS, GKMATERIALS_SIZE, 0);
		mgr.addResourceLocation("gkmaterials.zip", "EmbeddedZip",  Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,true);
...
bool gkResourceGroupManager::createResourceGroup(const gkResourceNameString& group, bool inGlobalPool)
{
	...
	try 
	{
		...
		gkLogger::write("Create Group:"+group.str(),true);
		m_groups.push_back(group);
		gkLogger::write("size:"+gkToString((int)m_groups.size()),true);
	}
	catch (Ogre::Exception& e)
	{
		gkLogMessage("ResourceGroupManager: " << e.getDescription());
...
void gkResourceGroupManager::destroyResourceGroup(const gkResourceNameString& group)
{
	...
	gkGroupManager::getSingleton().destroyGameObjectInstances(group.str());
	...
	gkLogger::write("erase Group:"+group.str(),true);
	m_groups.erase(m_groups.find(group));
	gkLogger::write("after erase size:"+gkToString((int)m_groups.size()),true);
}
...
bool gkResourceGroupManager::initRTShaderSystem(const gkString& shaderLang, const gkString& shaderCachePath, bool hasFixedCapability)
{
	...
	{
// TODO: Check this regarding gles2
...
gkString gkResourceGroupManager::getPathFromResource(const gkString& resourceName,const gkString& resourceGroupName) {
	Ogre::ResourceGroupManager::LocationList list = Ogre::ResourceGroupManager::getSingletonPtr()->getResourceLocationList(resourceGroupName);

	for( Ogre::ResourceGroupManager::LocationList::iterator it = list.begin(); it != list.end(); it++ )
	{
	  Ogre::String pathName = (*it)->archive->getName() + "/" + resourceName;
	  gkPath path(pathName);
	  if(path.exists()) {
		 return path.getAbsPath();
	  }
	}
