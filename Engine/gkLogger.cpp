...
void gkLogger::enable(const gkString& name, bool verbose)
{
	if (!gLog)
	{
		...
		if (gLog && Ogre::LogManager::getSingletonPtr())
		{
			gLog->setDebugOutputEnabled(true);
		}
...
//	}
	if (gLog && Ogre::LogManager::getSingletonPtr())
	{
		gLog->setDebugOutputEnabled(false);
	}
}

void gkLogger::logScreenConsole(const gkString& msg)
{
	write(msg,true);
	gkDebugScreen::printTo(msg);
}
