...
void gkEntity::createInstanceImpl(void)
{
	...
	if (m_meshName.getName().empty())
		m_meshName = m_entityProps->m_mesh->getResourceName();

	if (!m_entityProps->m_mesh && m_meshName.getName().empty())
		return;
...
//TODO: Check what happend with progressionlodmesh in new ogre

// create LOD for this mesh if not already done
//	if (hasVariable("gk_lod"))
//	{
//		m_entityProps->m_mesh->createLOD(false);
//	}

	Ogre::SceneManager* manager = m_scene->getManager();

	m_entity = manager->createEntity(m_name.getName(), m_meshName.getFullName(),
...
//
//	getOwner()->getPBSManager()->createHLMSMaterial(this->getEntity(),pbsMaterial,"pbsmat_"+getName());
}

Ogre::Entity* gkEntity::getEntity(bool reviveIfEmpty)
{
	if (!m_entity && reviveIfEmpty)
	{
		m_entity = m_scene->getManager()->createEntity(m_name.getName(), m_meshName.getFullName(),
				Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME);
	}
	return m_entity;
}
...
void gkEntity::_destroyAsStaticGeometry(void)
{
	if (m_entity != 0)
	{
		...
		// keep the entity if we want to update the staticbatch again
...
void gkEntity::setMaterialName(const gkString& matName) {
	if (m_entity && m_materialNameCache!=matName) {
		m_entity->setMaterialName(matName);
		m_materialNameCache = matName;
	}
}

void gkEntity::notifyUpdate(void)
{
	gkGameObject::notifyUpdate();
}
