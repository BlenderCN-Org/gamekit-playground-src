...
class gkGameObject : public gkInstancedObject, public gkTweenManager::gkTweenCallback
{
public:
...
public:
	...
	int _attachMoveable(Ogre::MovableObject* renderable);
	void _removeMoveables();
	void _removeMoveable(int pos);
	...
	GK_INLINE void			 attachVehicle(gkVehicle* vehicle) {m_vehicle = vehicle;}
	GK_INLINE gkVehicle*     getAttachedVehicle(void) 		{return m_vehicle;}
	...
	void tweenToPos(const gkVector3& to,float ptime,short ptransition = tween::EXPO, short pequation = tween::EASE_OUT, float pdelay = 0);
	...
	void		setGravity(const gkVector3& v);
	gkVector3   getGravity(void);



#ifdef OGREKIT_USE_NNODE
	void attachLogic(gkLogicTree* tree);
	gkLogicTree* getLogic() { return m_logic;}
	GK_INLINE gkString getLogicTreeName() { return m_logicTreeName;}
	GK_INLINE void setLogicTreeName(const gkString& name) { m_logicTreeName = name;}
#endif
	...
	GK_INLINE void setVisible(bool v)          {if (m_node) m_node->setVisible(v, true);}
	GK_INLINE void setVisibleRecursive(bool v) {if (m_node) m_node->setVisible(v, true);}
	GK_INLINE bool isVisible()                {return getNode()->getAttachedObject(0)->getVisible();}
	...
	gkBone* getBoneParent() { return m_boneParent;}
	void _setBoneParent(gkBone* bone) { m_boneParent = bone;}
protected:
	...
	// the bone this gameobject is parented to
	gkBone* 					m_boneParent;
	...
	typedef utList<Ogre::MovableObject*> MoveableList;
	MoveableList 				m_moveableList;
