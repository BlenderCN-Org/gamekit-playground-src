	...
	:    m_mesh(me)
{
	...
	const gkString& name = m_mesh->getResourceName().getFullName();



	Ogre::MeshPtr omesh = mgr.getByName(name).staticCast<Ogre::Mesh>();
	if (omesh.isNull())
		omesh = mgr.create(name, me->getGroupName(), true, this).staticCast<Ogre::Mesh>(); //GK_BUILTIN_GROUP
}
...
{
	Ogre::MeshManager& mgr = Ogre::MeshManager::getSingleton();
	mgr.remove(m_mesh->getResourceName().getFullName());
}
