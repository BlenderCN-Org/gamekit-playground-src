	...
	: m_material(material)
{
	...
	Ogre::MaterialPtr ptr = Ogre::MaterialManager::getSingleton().create(m_material.m_name, group).staticCast<Ogre::Material>(); //GK_BUILTIN_GROUP);

	ptr->setLightingEnabled(false);
	ptr->setReceiveShadows(false);


	Ogre::TexturePtr txptr =  Ogre::TextureManager::getSingleton().create(m_material.m_name, group, true, this).staticCast<Ogre::Texture>(); //GK_BUILTIN_GROUP
