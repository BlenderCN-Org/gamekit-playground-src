...
gkGroupManager::Groups gkGroupManager::getSceneGroups(gkScene* sc)
{
	UTsize pos;
	if ((pos = m_attachements.find(sc)) != UT_NPOS)
		return m_attachements.at(pos);
	return Groups();
}
...
void gkGroupManager::destroyGameObjectInstances(const gkString& groupName)
{
	GroupAttachements::Iterator it = m_attachements.iterator();
	while (it.hasMoreElements())
	{
		Groups::Iterator sceneGroups = it.getNext().second;
		while (sceneGroups.hasMoreElements())
			sceneGroups.getNext()->destroyGameObjectInstances(groupName);
	}
}
