...
void gkLuaScript::setFilePath(const gkString& path)
{
	m_fromFile = true;
	m_filePath = path;
}
...
bool gkLuaScript::execute(bool isHelperThread)
{
	gkLuaScript* lastScript = gkLuaManager::CURRENTSCRIPT;
	gkLuaManager::CURRENTSCRIPT = this;

	if (!m_compiled)
		compile();

	if (m_isInvalid){
		gkLuaManager::CURRENTSCRIPT = lastScript;
		return false;
	}
