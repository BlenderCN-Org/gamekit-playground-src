...
public:
	...
	static gkLuaScript* CURRENTSCRIPT;
	...
	gkLuaScript* createFromFile(const gkString& filename, const gkResourceName& name="");

	bool registerEvent(const gkString& name, gkLuaEvent* event);
	gkLuaEvent* getFunction(const gkString& name);
	// Destroys named file
	...
	void addPreparedLuaEvent(gkPreparedLuaEvent* prepEvent);

	void executePreparedLuaEvents();

	static gkString locateFileRelativeToCurrentScript(const gkString& path);
...
protected:
	...
	typedef utHashTable<utCharHashKey,gkLuaEvent*> LuaFunctionMap;
	LuaFunctionMap m_luaFuncMap;

	// this threadsafe queue will be filled e.g. via thrift/async-callbacks
	gkQueue<gkPreparedLuaEvent*> m_preparedLuaEvents;
};
