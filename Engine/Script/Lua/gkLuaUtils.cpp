...
void install_Templates(lua_State* L)
{
	...
	int ret = luaL_dostring(L, SYSTEM); // printf, dPrintf
	ret = luaL_dostring(L, CLASS);  // Class, BaseClass

//	ret = luaL_dostring(L, NGUIMAKER);  // automatic gui3d-translation
//	ret = luaL_dostring(L, JSON);
//
//	ret = luaL_dostring(L, PICKLETABLE);
//	ret = luaL_dostring(L, TABLEPERSISTANCE);
//	ret = luaL_dostring(L, UTILS);
//
//	ret = luaL_dostring(L, GAME);
//	ret = luaL_dostring(L, MESSAGEMANAGER);
//	ret = luaL_dostring(L, GROUPPOOL);
//	ret = luaL_dostring(L, UNDERSCORE);
//
//	ret = luaL_dostring(L, HELPERCLASSES);

//	ret = luaL_dostring(L, GZIP);

//	ret = luaL_dostring(L, CONTAINERS);
//	ret = luaL_dostring(L, WIRE_FORMAT);

//	ret = luaL_dostring(L, ENCODER);
//	ret = luaL_dostring(L, DECODER);
//	ret = luaL_dostring(L, DESCRIPTOR);
//	ret = luaL_dostring(L, LISTENER);
//	ret = luaL_dostring(L, TEXT_FORMAT);
//	ret = luaL_dostring(L, TYPE_CHECKERS);
//	ret = luaL_dostring(L, PROTOBUF);

//	ret = luaL_dostring(L, SOCKET);
//	ret = luaL_dostring(L, LTN12);
//	ret = luaL_dostring(L, URL);
//	ret = luaL_dostring(L, TP);
//	ret = luaL_dostring(L, FTP);
//	ret = luaL_dostring(L, HEADERS);
//	ret = luaL_dostring(L, HTTP);
//	ret = luaL_dostring(L, MBOX);
//	ret = luaL_dostring(L, MIME);
//	ret = luaL_dostring(L, SMTP);
...
extern "C" int luaopen_libluabpack(lua_State *L);
extern "C" int luaopen_libluabitwise(lua_State *L);
extern "C" int luaopen_liblualongnumber(lua_State *L);



void install_thrift(lua_State* L)
{
	luaopen_socket_core(L);
	luaopen_libluabpack(L);
	luaopen_libluabitwise(L);
	luaopen_liblualongnumber(L);
}
...
gkLuaEvent::gkLuaEvent(gkLuaCurState fnc)
	:   L(fnc.L), m_self(0), m_callback(0), m_callArgs(0), m_trace(-1), m_error(false),m_isHelperThread(false)
{
...
gkLuaEvent::gkLuaEvent(gkLuaCurState self, gkLuaCurState fnc,bool isHelperThread)
	:   L(fnc.L), m_self(0), m_callback(0), m_callArgs(0), m_trace(-1), m_error(false),m_isHelperThread(isHelperThread)
{
...
void gkLuaEvent::beginCall(void)
{
	...
	m_callArgs = 0;
	// testing if this can stop the stack-overflows(seems to work)
	lua_settop(L,0);
...
void gkLuaEvent::addArgument(const char* str)
{
	if(m_error) return;
...
void gkLuaEvent::addArgument(const gkString& str)
{
	if(m_error) return;
...
void gkLuaEvent::addArgument(const gkString& swigType,void* pntr, int swigOwner)
{
        lua_State* L = gkLuaManager::getSingleton().getLua();
        swig_type_info *outputType = SWIG_TypeQuery(L,swigType.c_str());
        SWIG_NewPointerObj(L, pntr, outputType, swigOwner);
        ++m_callArgs;
}

void gkPreparedLuaEvent::PreparedArgument::dispose() {
	switch (m_type) {
		case typeBool: 	delete (bool*)m_value;
						m_value = 0;
						break;
		case typeCharStar: // TODO: is that disposed the right way?
						delete (char*)m_value;
						m_value = 0;
						break;
		case typeFloat: 	delete (float*)m_value;
						m_value = 0;
						break;
		case typeGkString: 	delete (gkString*)m_value;
						m_value = 0;
						break;
		case typeInt: 	delete (int*)m_value;
						m_value = 0;
						break;
		case typeSwigTypeString:
						delete (gkString*)m_value;
						m_value = 0;
						delete (int*)m_value3;
						m_value3 = 0;
						break;
...
void gkPreparedLuaEvent::addArgument(bool val){
	m_preparedArguments.push_back(new PreparedArgument(typeBool,new bool(val)));
}
void gkPreparedLuaEvent::addArgument(int val){
	m_preparedArguments.push_back(new PreparedArgument(typeInt,new int(val)));
}
void gkPreparedLuaEvent::addArgument(float val){
	m_preparedArguments.push_back(new PreparedArgument(typeFloat,new float(val)));
}
void gkPreparedLuaEvent::addArgument(const gkString& val){
	m_preparedArguments.push_back(new PreparedArgument(typeGkString,new gkString(val)));
}
   // call this like this:
	    // addArgument("tsScriptClass *",pointer to instance,0=you are responsible for deletion 1=swig will kill it for you)
void gkPreparedLuaEvent::addArgument(const gkString& swigType,void* pntr,int swigOwner){
	PreparedArgument* arg = new PreparedArgument(typeSwigTypeString,new gkString(swigType));
	arg->setValue2(pntr);
	arg->setValue3(new int(swigOwner));
	m_preparedArguments.push_back(arg);
}

bool gkPreparedLuaEvent::call() {
	ArgumentList::Iterator iter(m_preparedArguments);
	gkLuaEvent::beginCall();
	while (iter.hasMoreElements()) {
		...
		switch (type) {
			case typeBool: 	gkLuaEvent::addArgument(*(bool*)arg->getValue1());
							break;
			case typeFloat:
							gkLuaEvent::addArgument(*(float*)arg->getValue1());
							break;
			case typeGkString:
							gkLuaEvent::addArgument(*(gkString*)arg->getValue1());
							break;
			case typeInt:
							gkLuaEvent::addArgument(*(int*)arg->getValue1());
							break;
			case typeSwigTypeString:
							gkLuaEvent::addArgument(*(gkString*)arg->getValue1(),arg->getValue2(),*(int*)arg->getValue3());
							break;
		}
		if (m_deleteAfterExecution)
			arg->dispose();
	}
	return gkLuaEvent::call();
}


void gkLuaEvent::incArgumentCount() {
        ++m_callArgs;
}
...
const char* gkLuaEvent::callString()
{

	if (m_error) return 0;
//	if (m_callArgs == 0) return 0;
...
int gkLuaEvent::callInt()
{

	if (m_error) return -1;
//	if (m_callArgs == 0) return -1;
...
bool gkLuaEvent::call()
{
	...
	//TODO: ??? why not call without args?
//	if (m_callArgs == 0) {
//		return false;
//	}
...
//gkSyncObj gkLuaEvent::sync;
//bool gkLuaEvent::m_scriptInProgress = false;
