...
gkGameObject* gsRayTest::getHitObject(void)
{
	return m_ray->getHitObject();
}
...
bool gsSweptTest::collides(const gsRay& ray, float rayRadius,int filterGroup,int filterMask)
{
	return m_test->collides(ray, rayRadius,filterGroup,filterMask);
}
