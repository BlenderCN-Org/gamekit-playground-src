...
-------------------------------------------------------------------------------
    ...
    Copyright (c) 2006-2012 Charlie C.
...
public:
	...
	gkLogicLink* getObject(gsGameObject* obj);
...
public:
	...
	gkString  getMessageSubject(int msgNr) { return msgNr<getMessageCount()?get()->getMessage(msgNr).m_subject:"OUTOFBOUNDS";}
	gkString  getMessageBody(int msgNr) { return msgNr<getMessageCount()?get()->getMessage(msgNr).m_body:"OUTOFBOUNDS";}
	gkString  getMessageFrom(int msgNr) { return msgNr<getMessageCount()?get()->getMessage(msgNr).m_from:"OUTOFBOUNDS";}
	gkString  getMessageTo(int msgNr) { return msgNr<getMessageCount()?get()->getMessage(msgNr).m_to:"OUTOFBOUNDS";}
...
public:
	...
	const int 	  getHitObjectCount(void)     {return get()->getHitObjectCount();}
	gkGameObject*  getHitObject(int nr)       {return get()->getHitObject(nr);}
...
public:
	...
	bool      isPlaying(void)			  {BRICK_GET( isPlaying(),false);}
