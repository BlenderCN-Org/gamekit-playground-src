...
public:
	...
	gkMathNode(gkLogicTree* parent, size_t id, const gkString& name="")
		: gkLogicNode(parent, id,name)
	{
		ADD_ISOCK_VAR(A, T());
		ADD_ISOCK_VAR(B, T());
		ADD_OSOCK_VAR(RESULT, T());
	}
	...
	T doMath(Int2Type<MTH_INVERSE>)
	{
		return m_a * -1;
	}

	T doMath(Int2Type<MTH_NO_FUNC>)
	{
		return m_a;
	}

	T doMath(Int2Type<MTH_SQR>)
	{
		return gkMath::Sqr(m_a);
	}

	T doMath(Int2Type<MTH_SQRT>)
	{
		return gkMath::Sqrt(m_a);
	}
