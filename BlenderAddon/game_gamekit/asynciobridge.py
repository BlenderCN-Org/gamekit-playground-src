...
runtimedata.RuntimeData.requestServerExit=False


# @persistent
# def load_handler(dummy):
#     if not runtimedata.RuntimeData.asycBridgeRunning:
#         print("TRY TO START ASYNC SERVER")
#         startAsyncServer()
#             
# bpy.app.handlers.load_post.append(load_handler)

def startAsyncServer():
    if not runtimedata.RuntimeData.asycBridgeRunning:
        bpy.ops.bpy.start_asyncio_bridge()
        runtimedata.RuntimeData.asycBridgeRunning = True
        print("START ASYNCIO BRIDGE:" + str(runtimedata.RuntimeData.asycBridgeRunning))
    else:
        print("ALREADY RUNNING")

def stopAsyncServer():
    if runtimedata.RuntimeData.asycBridgeRunning:
        runtimedata.RuntimeData.requestServerExit=True
...
class AsyncioBridgeOperator(bpy.types.Operator):
            ...
            for listener_id, listener in self.listeners.items():
                ...
                if fire:
                    ...
                    if catch:
                        ...
                        return {'PASS_THROUGH'}
        if runtimedata.RuntimeData.requestServerExit:
            print("FINISH HIM")
            self.cancel(context)
            runtimedata.RuntimeData.asycBridgeRunning = False
            runtimedata.RuntimeData.requestServerExit=False
    ...
    def execute(self, context):
        AsyncioBridgeOperator.running=True
        self.types = {}
        self.listeners = {}
        self.listener_id = 0
