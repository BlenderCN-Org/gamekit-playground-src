...
{
	if (m_base->hasStopRequest())
	{
		if (!m_base->hasOverrideStop())
		{
			// since there is no override-request from somewhere else, the animation is removed from the blender
			m_base->clearRequests();
			return true;
		}
	}
	m_base->clearRequests();
	...
	if (m_way == akAnimationBlend::AB_OUT && (1.f - m_blend) <= 0.f)
		return true;
	
	if (m_mode == AK_ACT_END && m_time>= getLength())
	{
		return true;
	}
...
//				ab.setMode(AK_ACT_LOOP);
			...
			if (ab.evaluate(delta))
			{
				...
				ab.getAnimationPlayer()->fireAnimationEnd();
			}
		...
		while (i < s)
		{
			if (m_stack.size() > 1) //    Why does it want to have one inside the stack?
			{
				p[i].reset();
				m_stack.erase(p[i]);
			}
