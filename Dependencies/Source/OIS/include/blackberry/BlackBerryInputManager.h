...
#ifndef OIS_BlackBerryInputManager_H
#define OIS_BlackBerryInputManager_H

namespace OIS
{

class BlackBerryKeyboard : public Keyboard
{
	char mKeyBuffer[256];
public:
	BlackBerryKeyboard(InputManager* creator);
...
public:
	BlackBerryMultiTouch(InputManager* creator) :
			OIS::MultiTouch(creator->inputSystemName(), false, 0, 0), mOffsetX(0), mOffsetY(0) {
...
class BlackBerryAccelerometer : public JoyStick
{
public:
	BlackBerryAccelerometer(InputManager* creator, bool buffered);
	virtual ~BlackBerryAccelerometer();
...
class BlackBerryInputManager : public InputManager, public FactoryCreator
{
	BlackBerryMultiTouch* mTouch;
	BlackBerryKeyboard* mKeyboard;
	BlackBerryAccelerometer* mAccelerometer;

public:
	BlackBerryInputManager();
	~BlackBerryInputManager();
	...
	void injectKey(int action, int uniChar, int keyCode)
	{
		if (mKeyboard) mKeyboard->injectKey(action, uniChar, keyCode);
	}
