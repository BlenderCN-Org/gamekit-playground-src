...
struct fbtMapping
{
	const FBTuint16   blendCode;
	const FBTuint16   datCode;
};
...
fbtMapping blend2datMapping[] =
{
	{ FBT_ID2('S', 'C'), FBT_ID2('M', 'E')},
	{ FBT_ID2('M', 'E'), FBT_ID2('S', 'C')},
	{ FBT_ID2('S', 'N'), FBT_ID2('O', 'B') },
	{ FBT_ID2('O', 'B'), FBT_ID2('S', 'N') },
	{ FBT_ID2('T', 'E'), FBT_ID2('M', 'A') },
	{ FBT_ID2('M', 'A'), FBT_ID2('T', 'E') },
	{ 0, 0 }
};
...
fbtBlend::fbtBlend()
	:
		...
		, m_stripList(0)
{
	m_aluhid = "ToMaGa0"; //a stripped blend file
}
...
int fbtBlend::notifyData(void* p, const Chunk& id)
{
	...
	FBTuint16 alternateCode = 0;
	if (isAlternativeFile()){
		alternateCode = getID_DatToBlend(id.m_code);
	}
	...
	if ((id.m_code <= 0xFFFF))
	{
		...
		{
			if ((alternateCode && fbtData[i].m_code == alternateCode)
					|| (!alternateCode && fbtData[i].m_code == id.m_code) )
			{
...
FBTuint32 fbtBlend::getID_BlendtoDat(FBTuint32 code) {
	int i = 0;
	while (blend2datMapping[i].blendCode!=0){
		if (code == blend2datMapping[i].blendCode){
			return blend2datMapping[i].datCode;
		}
		i++;
	}
	return code;
...
FBTuint32 fbtBlend::getID_DatToBlend(FBTuint32 code) {
	int i = 0;
	while (blend2datMapping[i].blendCode!=0){
		if (code == blend2datMapping[i].datCode){
			return blend2datMapping[i].blendCode;
		}
		i++;
	}
	return code;
...
int fbtBlend::writeData(fbtStream* stream)
{
	...
	for (MemoryChunk* node = (MemoryChunk*)m_chunks.first; node; node = node->m_next)
	{
		...
		if (!isAlternativeFile())
			...
			ch.m_code   = getID_BlendtoDat(node->m_chunk.m_code);
