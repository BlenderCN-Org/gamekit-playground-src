...
const FBTuint32 alternateENDB = FBT_ID('D', 'B', 'E', 'B');
const FBTuint32 alternateDNA1 = FBT_ID('1', 'N', 'D', '1');
const FBTuint32 alternateDATA = FBT_ID('A', 'A', 'T', 'Q');
const FBTuint32 alternateSDNA = FBT_ID('G', 'G', 'A', 'N');
...
fbtFile::fbtFile(const char* uid)
	:   m_version(-1), m_fileVersion(0), m_fileHeader(0), m_uhid(uid), m_aluhid(0),
	    m_memory(0), m_file(0), m_curFile(0), m_isAlternativeFile(0)
{
}
...
int fbtFile::parseHeader(fbtStream* stream, bool suppressHeaderWarning)
{
	...
	m_isAlternativeFile = fbtCharNEq(m_header.c_str(), m_aluhid, 7);
...
int fbtFile::parseStreamImpl(fbtStream* stream, bool suppressHeaderWarning)
{
	...
	do
	{
		...
		if ((m_isAlternativeFile && chunk.m_code == SDNA)
			|| (!m_isAlternativeFile && chunk.m_code == alternateSDNA) )
		{
			chunk.m_code = !m_isAlternativeFile?DNA1:alternateDNA1;
			stream->seek(-status, SEEK_CUR);
			chunk.m_len = stream->size() - stream->position();
		}

		// exit on end byte
		if ( (!m_isAlternativeFile && chunk.m_code == ENDB)
			|| (m_isAlternativeFile && chunk.m_code == alternateENDB))
			break;
		...
		if ( (!m_isAlternativeFile && chunk.m_code == DNA1)
			||(m_isAlternativeFile && chunk.m_code == alternateDNA1))
		{
...
//	}
	...
	// write DNA1
	Chunk ch;
	ch.m_code   = !m_isAlternativeFile?DNA1:alternateDNA1;
	...
	ch.m_code   = !m_isAlternativeFile?ENDB:alternateENDB;
...
{
	Chunk ch;
	ch.m_code   = !m_isAlternativeFile?DATA:alternateDATA;
	ch.m_len    = len;
	ch.m_nr     = 1;
	ch.m_old    = (FBTsize)writeData;
