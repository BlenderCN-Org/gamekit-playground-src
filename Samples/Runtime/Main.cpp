...
#include "gkMotionNodeImpl.h"
#include "gkMouseNodeImpl.h"
#include "gkMathNodeImpl.h"
#include "gkTimerNodeImpl.h"

#include "gkNodeTreeNodeImpl.h"
#if defined(RELEASE_MODE) && (GK_PLATFORM == GK_PLATFORM_WIN32)
#define WIN32_LEAN_AND_MEAN
# include <windows.h>
#endif

const gkString gkDefaultBlend   = "game.dat";
...
int OgreKit::setup(int argc, char** argv)
{
	...
	try
	{
		...
		TCLAP::ValueArg<bool>			esctoexit_arg			("", "esctoexit",				"Use esc-key to exit the application.", false, m_prefs.esctoexit, "bool");
		TCLAP::ValueArg<std::string>			injectlua_arg			("e", "luainject",				"Inject lua to be executed before anything else is started", false, m_prefs.injectedLua, "string");
		...
		cmdl.add(esctoexit_arg);
		cmdl.add(injectlua_arg);
		...
		m_prefs.esctoexit				= esctoexit_arg.getValue();
		m_prefs.injectedLua				= injectlua_arg.getValue();
		m_prefs.matblending				= matblending_arg.getValue();
		// if we inject lua, we think it is because of the debugger. therefore we automatically release the input.
		// Maybe this have to be rethought someday.
		m_prefs.grabInput				= !m_prefs.injectedLua.empty()?false:grapInput_arg.getValue();
	...
	#else
	m_prefs.winsize = gkVector2(1024,768);
	gkString cfgfname(gkDefaultConfig);
	m_blend = gkDefaultBlend;
	...
	if (!m_prefs.shaderCachePath.empty()) {
		gkPath shaderPath(m_prefs.shaderCachePath);
		if (!shaderPath.exists()) {
			std::cerr << "shaderCachePath("<<m_prefs.shaderCachePath<<") does not exist!";
			return -1;
		}
	}
...
bool OgreKit::setup(void)
{
	...
	gkPath inputPath(gkUtils::getFile(m_blend));

	gkString ext = inputPath.extension();

	Gamekit_addNodeInstancer();
	// add manual node-instancers
...
//	gkNodeTreeConverter::addNodeInstancer(new CustomTimerNodeInstancer());

	if (inputPath.extension()==".lua")
	{
		gkLogger::write("Try to load LUA:" + inputPath.getPath(),true);
		// load lua-file instead of start from blend

		// with this hack the debugger might also be able to process breakpoints in the cli-startup-script
//		gkLuaScript* startScript = gkLuaManager::getSingleton().createFromText("cli-startup","Gk.import('"+inputPath.getPath()+"')   ");
		gkLuaScript* startScript = gkLuaManager::getSingleton().createFromFile(inputPath.getPath());
		if (!startScript) {
			gkLogger::write("ERROR: Couldn't locate "+inputPath.getPath(),true);
			return false;
		}
		startScript->execute();
		...
		if (!m_scene)
		{
			m_scene = gkSceneManager::getSingleton().createEmptyScene("default","camera","default");
			m_scene->createInstance();
		}
	}
	else
	{
		gkLogger::write("Try to load BLEND:" + inputPath.getPath(),true);
		gkBlendFile* blend = gkBlendLoader::getSingleton().loadFile(inputPath.getPath(), gkBlendLoader::LO_ALL_SCENES);
		if (!blend)
		{
...
{
	if (sc == KC_ESCKEY && m_prefs.esctoexit){
		gkAddonAdder::removeAddons();
		m_engine->requestExit();
	}
}

#if defined(RELEASE_MODE) and GK_PLATFORM == GK_PLATFORM_WIN32
...
int WINAPI WinMain(   HINSTANCE   hInstance,HINSTANCE   hPrevInstance,LPSTR   lpCmdLine,int nCmdShow)
{
	...
	gkLogger::write("WINAPI USED!",true);
